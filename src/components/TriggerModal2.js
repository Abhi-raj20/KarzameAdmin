...
const TriggerModal2 = React.memo(
  ({ icon, triggerTitle, onCloseTrigger, triggerModal, triggerModalData }) => {
    ...
    useEffect(() => {
      if (triggerModalData && triggerModalData.latitude && triggerModalData.longitude) {
        getAddressFromCoordinates(triggerModalData.latitude, triggerModalData.longitude);
      }
    }, [triggerModalData]);
      ...
      >
        <Box className="col-md-9 col-sm-9 col-lg-6" style={styles.modalContainer}>
          ...
          {icon ? (
            ...
            >
              <BatteryIcon percentage={triggerModalData.BatteryLevel} />
            </div>
          )}
          ...
          {renderInfo("Name", triggerModalData.userName)}
          {renderInfo("Date", triggerModalData.Date)}
          {renderInfo("Phone No", triggerModalData.userPhone)}
          {renderInfo("State", triggerModalData.State)}
          {renderInfo("City/Town", triggerModalData.City)}
          {renderInfo("User Status", triggerModalData.IsMoving ? "Moving" : "Not Moving")}
