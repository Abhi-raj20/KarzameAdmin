  ...
  const getSos = async () => {
    ...
    onValue(usersRef, (snapshot) => {
      ...
      if (snapshot.exists()) {
            ...
            .map(([userId, user]) => {
              if (user && user.SOS) {
                if (
                  ...
                  return Object.entries(user.SOS).map(([sosId, sosItem]) => {
                    return {
        ...
        // Sort the sosItems array based on the 'Date' property in descending order
        sosItems.sort((a, b) => new Date(b.Date) - new Date(a.Date));

        // Add SR to sosItems
        sosItems.forEach((sosItem, index) => {
          sosItem.sr = index + 1;
        });

        // Now sosItems contains data for all SOS entries sorted by the latest at the top
        console.log("sosItems ", sosItems);
        setRows(sosItems);
        setLoading(false);
      } else {
        setLoading(false);
