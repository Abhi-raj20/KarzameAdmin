...
import soundFile from "../../Raw/ring.wav";
...
function ZonePassed() {
  ...
  const deleteSosItem = async (userId, sosId) => {
    ...
    const usersRef = ref(db, `users/${userId}/GeoFence/${sosId}`);

    try {
      // Remove the SOS item from the database
      await remove(usersRef);
      console.log("Geo fence item deleted successfully");

      // Call getSos after the deletion is completed
      getGeoFence();
    } catch (error) {
      console.error("Error deleting geo fence item:", error);
    }
  };
  ...
  const getGeoFence = async () => {
    ...
    onValue(usersRef, (snapshot) => {
      ...
      if (snapshot.exists()) {
        ...
        const geoFenceItems = Array.from(
            ...
            .map(([userId, user]) => {
              if (user.GeoFence) {
                if (
                  ...
                  return Object.entries(user.GeoFence).map(([sosId, sosItem]) => {
                    return {
        ...
        // Sort the geoFenceItems array based on the 'Date' property in descending order
        geoFenceItems.sort((a, b) => new Date(b.Date) - new Date(a.Date));

        // Add SR to geoFenceItems
        geoFenceItems.forEach((sosItem, index) => {
          sosItem.sr = index + 1;
        });

        // Now geoFenceItems contains data for all SOS entries sorted by the latest at the top
        console.log("GeoFence ", geoFenceItems);
        setRows(geoFenceItems);
        setLoading(false);
      } else {
        setLoading(true);
                ...
                <Card>
                  ...
                  >
                    <MDTypography variant="h6" color="white">
                      Virtual Guard Alert Zone Passed
                    </MDTypography>
                  </MDBox>
                  <MDBox pt={3}></MDBox>
